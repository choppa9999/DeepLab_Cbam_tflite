...
class SegmentationDataset:
    ...
    def _determine_num_classes(self, file_list, num_samples_to_check=200):
        ...
        unique_labels = set([0])  # 배경 클래스(0)는 항상 존재한다고 가정
        for _, label in dataset:
            # 라벨에서 유니크한 값들을 찾습니다.
            # ignore_label(255)는 제외합니다.
        ...
        # 클래스 수는 (최대 레이블 ID + 1) 입니다.
        # 예를 들어, 레이블이 0, 1, 2이면 max는 2이고, 클래스 수는 3개입니다.
    ...
    def _get_data_files(self):
        ...
        if self.subset == 'trainval':
            train_pattern = os.path.join(self.dataset_dir, f'train-*-of-*.tfrecord')
            val_pattern = os.path.join(self.dataset_dir, f'valid-*-of-*.tfrecord')
            data_files = tf.io.gfile.glob(train_pattern) + tf.io.gfile.glob(val_pattern)
        else:
            file_pattern = os.path.join(self.dataset_dir, f'{self.subset}-*-of-*.tfrecord')
            data_files = tf.io.gfile.glob(file_pattern)

        if not data_files:
            raise FileNotFoundError(
                f"TFRecord 파일을 찾을 수 없습니다. 경로와 subset 이름을 확인하세요: {self.dataset_dir}, subset: {self.subset}")
